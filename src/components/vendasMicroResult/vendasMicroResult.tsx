...
async function VendaMicro() {
  ...
  async function getVenda() {
    ...
    const url = `http://192.168.30.252:9091/api_crm/vendas/vendas_detalhado.php?dataInicial='${dataInicial}'&dataFinal='${dataFinal}'`;
  ...
  const fetchData = async () => {
    ...
    await getVenda().then((responseJson) => {
      setDatas(responseJson.VENDADET);
          ...
          <tbody className="text-gray-700">
            <tr>
              ...
              <td className="w-1/3 text-left py-3 px-4 divide-y divide-solid">
                {" "}
                {Object.values(datas).map((val, key) => {
                  return <div key={key}>{val.PESNOME}</div>;
                })}
              </td>
              <td className="w-1/3 text-left py-3 px-4 divide-y divide-solid">
                {" "}
